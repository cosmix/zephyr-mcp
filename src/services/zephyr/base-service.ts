     ...
     try {
      ...
      if (contentType && contentType.includes("application/json")) {
       ...
       if (error instanceof McpError) {
         ...
         console.error(`[Zephyr MCP] McpError ${error.code}: ${error.message}`, error.data);
         throw error;
       }
 
       
       console.error(`[Zephyr MCP] Unexpected error during request: Method=${method}, Path=${path}`, error);
       throw new McpError(
